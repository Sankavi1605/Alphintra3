...
for (const [filename, pos] of Object.entries(sections)) {
  ...
  if (w <= 900) {
    ...
    var f = w / 1920;
    // CRITICAL FIX: The available empty space between the card and the screen edge shrinks FASTER than the screen itself.
    // By using a non-linear power scale (f^1.4), the text naturally scales down aggressively enough to fit 
    // inside the tight sliver of space on square/portrait tablet displays, without overflowing either side.
    var s = Math.max(0.30, Math.pow(f, 1.4));
    text.el.scale.set(s, s, s);
