...
const sections = {
  'ballSection.js': { origX: 15, origY: 0, origZ: 15, origRotX: 0, origRotY: -0.4, mobileX: -6, selector: '.heads__section--ball' },
  'beamsSection.js': { origX: -10, origY: 8, origZ: 0, origRotX: 0, origRotY: 0, mobileX: 6, selector: '.heads__section--beams' },
  'dropSection.js': { origX: -10, origY: 8, origZ: 0, origRotX: 0, origRotY: 0, mobileX: 6, selector: '.heads__section--drop' },
  'endSection.js': { origX: 0, origY: 0, origZ: 0, origRotX: 0, origRotY: 0, mobileX: 0, selector: '.heads__section--end' },
  'faceSection.js': { origX: 26, origY: 0, origZ: 0, origRotX: 0, origRotY: -0.4, mobileX: -6, selector: '.heads__section--face' },
  'flowSection.js': { origX: 0, origY: 0, origZ: -10, origRotX: 0, origRotY: 0.4, mobileX: 0, selector: '.heads__section--flow' },
  'galaxySection.js': { origX: 0, origY: 20, origZ: -20, origRotX: 0, origRotY: 0, mobileX: 0, selector: '.heads__section--galaxy' },
  'heightSection.js': { origX: -20, origY: 0, origZ: 0, origRotX: 0, origRotY: 0, mobileX: 6, selector: '.heads__section--height' },
  'rocksSection.js': { origX: 0, origY: 0, origZ: 0, origRotX: 0, origRotY: 0, mobileX: 0, selector: '.heads__section--rocks' },
  'waveSection.js': { origX: 0, origY: 10, origZ: 0, origRotX: 0.2, origRotY: 0, mobileX: 0, selector: '.heads__section--wave' }
};
...
for (const [filename, pos] of Object.entries(sections)) {
  ...
  if (w <= 900) {
    ...
    if (cardEl) {
      ...
      // Only consider it if the card is visible and takes up significant space
      if (rect.width > 0 && rect.height > 0) {
        // Map pixel coordinates to 3D coordinates at textZ depth!
        var rectLeft3D = ((rect.left / w) * W_3D) - E;
        var rectRight3D = ((rect.right / w) * W_3D) - E;
        
        // Add 80px buffer for UI controls like slider arrows
    ...
    // text.width3D is the actual width of the text plane in 3D units (added in V11)
    var actualTextWidth = text.width3D || 25; // fallback to 25 if width3D not available
    
    // Calculate how much rotation shrinks the apparent width (cos of angle)
    ...
    // 6. Interpolate X position and Scale
    var progress = Math.max(0, Math.min(1, (1920 - w) / 900));
    var safeX = isCenter ? 0 : ${pos.origX} + (trueCenter - ${pos.origX}) * progress;
    
    var f = w / 1920;
    var designerS = Math.max(0.30, Math.pow(f, 1.2));
    
    // Use the MINIMUM of the designer's scale and the strictly required scale
