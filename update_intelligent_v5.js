...
for (const [filename, pos] of Object.entries(sections)) {
  ...
  // Restore v2 values (4 / -4) which correctly kept text AWAY from the cards.
  let targetX = 0;
  ...
  if (w <= 900) {
    ...
    // Use the exact original Y so we don't accidentally push text off the top/bottom
    ...
    var f = w / 1920;
    // Allow scale to drop to 0.45 on narrow screens so wide text fits within edges
