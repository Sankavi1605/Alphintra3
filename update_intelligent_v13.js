...
for (const [filename, pos] of Object.entries(sections)) {
  ...
  if (w <= 900) {
    ...
    // Planet avoidance logic (if there is a large 3D object in the center)
    var planetRadius = ${pos.planetRadius};
    if (planetRadius > 0) {
      var planetBuffer = 3; // Extra buffer around planet
      if (isLeftCard) {
        // Text is on the right, card is on the left
        leftBoundary3D = Math.max(leftBoundary3D, planetRadius + planetBuffer);
      } else if (isRightCard) {
        // Text is on the left, card is on the right
...
window.addEventListener('resize', updateTextPosition);`;
  ...
  if (existingFuncRegex.test(content)) {
    ...
    // FIX REGEX: Use exact string replacement to avoid RegExp escape hell!
